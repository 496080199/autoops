...
class ConfHandler(BaseHandler):
    ...
    def post(self,env_id,prod_id): 
        ...
        if int(conf.time) == 1:
            ...
            if ver:
                job = user_cron.new(command="cd "+upload_path+"&&ansible-playbook "+prod_id+".yml -i "+prod_id+".host -e \"bag="+ver.file+"\" | tee cronlogs/cron_$(date +%Y%m%d%H%I%S).log", comment='autoops_'+prod.name+'_'+prod_id)
